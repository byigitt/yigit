...
export function LocalAgentNotificationsBridge() {
  ...
  useEffect(() => {
    ...
    if (status === "awaiting-approval") {
      fire("attention", "yigit needs your approval", "Approve a tool to continue");
    } else if (status === "error") {
      fire("error", "yigit run failed", error ?? undefined);
    } else if (status === "idle" && isBusy(was)) {
      fire("finished", "yigit finished", "Your task is ready");
    }
  }, [status, error]);
