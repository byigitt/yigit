...
/**
 * Replace `targetId` with a split that contains the original leaf and an
 * arbitrary `incoming` subtree. Used when a tab is dragged onto an existing
 * pane to merge it as a sibling split.
 *
 * `position` determines which side of the target the incoming subtree lands
 * on. If the target's enclosing split already runs in `dir`, the incoming
 * subtree is spliced as a flat sibling instead of producing a nested split.
 */
export function mergeIntoLeaf(
  tree: PaneNode,
  targetId: PaneId,
  incoming: PaneNode,
  newSplitId: PaneId,
  ...
  if (tree.kind === "split" && tree.dir === dir) {
    ...
    if (idx >= 0) {
      ...
      return {
        ...tree,
        children: [
          ...tree.children.slice(0, insertAt),
          incoming,
          ...tree.children.slice(insertAt),
        ],
      };
    }
  }
  if (isLeaf(tree)) {
    ...
    const children: PaneNode[] =
      position === "before" ? [incoming, tree] : [tree, incoming];
    return { kind: "split", id: newSplitId, dir, children };
  }
  return {
    ...tree,
    children: tree.children.map((c) =>
      mergeIntoLeaf(c, targetId, incoming, newSplitId, dir, position),
    ),
  };
}
